	...
	function Orion:SaveConfig(k, v) Config[k] = v end
	function Orion:GetConfig(k) return Config[k] end

function Orion:Notify(title, text, duration)
	duration = duration or 3

	if not NotifyGui then
		NotifyGui = Instance.new("ScreenGui")
		NotifyGui.Name = "OrionNotify"
		...
		function Window:CreateTab(name)
			...
			function Tab:CreateSection(secName)
				...
				function Section:Toggle(text, callback)
					local state = Orion:GetConfig(text) or false
...
-- window
local Window = Orion:CreateWindow("Onion Hub 🧅")
...
local Farm     = FarmTab:CreateSection("Auto Farm")
...
local PlayerStats = MainInfo:Embed({
	...
	Fields = {
		...
		{Name = "Game Time",       Value = "00:00"},
		{Name = "Currency",        Value = "$0"},
		{Name = "Job",             Value = "Nganggur"},
		{Name = "Current Players", Value = "1"}
	}
})
...
task.spawn(function()
	...
	PlayerStats:UpdateField("Job", "Warteg Owner")
	PlayerStats:UpdateField("Game Time", "14:30")
	PlayerStats:UpdateField("Current Players", #game.Players:GetPlayers())
end)
...
-- farm toggle
Farm:Toggle("Enable Farm", function(state)
	print("Farm state:", state)
end)

-- notify
Orion:Notify("Loaded","🧅 CDID Hub Loaded",8)
